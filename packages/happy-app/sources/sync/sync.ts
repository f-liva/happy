...
class Sync {
    ...
    private fetchSessions = async () => {
        ...
        const { ordered, processedCount } = await this.ingestSessions(sessions);
     ...
     * newest first, in chunks. Shared by the boot fetch (150 most recent) and
     * the on-demand history load behind the search box.
     */
    ...
    // the already-merged store. Pages go through the same chunked ingest as
    // boot so the list stays responsive while history fills in.
    ...
    public loadAllSessionsForSearch = (): Promise<void> => {
        ...
        this.allSessionsLoaded = (async () => {
            ...
            do {
                ...
                if (fresh.length > 0) {
                    const { processedCount } = await this.ingestSessions(fresh);
